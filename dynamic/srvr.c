...
int leader_election();
void udp_multicast(char *msg, struct serverInfo *head);
...
int main()
{
	...
	SOCKET udp_socket = setup_udp_socket(SERVER_IP, PORT);
	if (udp_socket!=-1)
	{
	...
	while (1)
	{
        ...
        {
			if (leader == 1)
				udp_multicast(msg1, connected_peers);
			else
            	udp_multicast(msg2, connected_peers);
...
void udp_broadcast(char *msg)
{
	...
	if (brd_socket != -1)
	{
		...
		if (sendto(brd_socket, msg, strlen(msg), 0, rcv_addr->ai_addr, rcv_addr->ai_addrlen) == -1)
...
void handle_udp_recieve(ServerInfo *connected_peers, int leader, SOCKET udp_socket)
{
	...
	if (leader == 1)
	{
		...
		{
			printf("[Leader UDP] Received: (%d) bytes: %.*s\n", bytes_received, bytes_received, read);
